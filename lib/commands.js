...
const { extractAgendaItems } = require('./agenda');
const { SHORT: QUOTA_SHORT } = require('./quota');
...
// 一次撤銷整批自動排進行事曆的項目。
// postback 的 data 有長度限制，塞不下好幾個 UUID，所以存檔時給整批一個
// batch id（寫在 meta.agenda_batch），撤銷時照 batch id 一起刪。
function buildAgendaUndoReply(text, batchId) {
  return {
    type: 'flex',
    altText: text,
    contents: {
      ...
      footer: {
        ...
        contents: [{
          type: 'button', style: 'secondary', height: 'sm',
          action: {
            type: 'postback', label: '🗑 取消排進行事曆',
            data: `act=delagenda&b=${batchId}`,
            displayText: '取消剛剛排進行事曆的項目',
          },
        }],
      },
    },
...
async function handleSave(text) {
  ...
  // AI 沒跑成功時要講出來。不講的話使用者會收到一則看起來完全正常的
  // 存檔確認，但其實分類沒做、時間也沒抓到。
  const notes = [...warnings];
  if (classified.ai_failed) {
    notes.push(classified.quota_exhausted
      ? `⚠️ ${QUOTA_SHORT}`
      : '⚠️ AI 這次沒跑成功，先照原文存起來了。時間或分類要調整的話可以用「改時間」「改內容」。');
  }
  if (!notes.length) return reply;

  // buildSaveReply 回的是 Flex 物件，直接串進樣板字串會變成 [object Object]，
  // 所以要用陣列讓 lib/line.js 拆成兩則訊息送。
...
const HELP_SECTIONS = {
  ...
  tools: [
    ...
    '・第一行會當成會議標題，清單上就看得出是哪場會議',
    '・內容裡有日期的議程會自動排進行事曆（可一鍵取消）',
    '',
    '📷 相簿',
    '・傳過的圖都會自動留檔，到後台「相簿」看',
...
const REPORT_PROJECT = '週報';
const SYSTEM_PROJECTS = [ORDER_PROJECT, SONG_PROJECT, TEMPLATE_PROJECT, LINK_PROJECT, MEETING_PROJECT, ALBUM_PROJECT, REPORT_PROJECT];
...
// 會議標題：取內容第一行。
// 使用者本來就習慣把會議名稱寫在第一行（實際 9 筆記錄全都是這樣），
// 所以不用另外開欄位、也不用改既有資料，直接推導就好。
...
async function saveMeeting(dateISO, content) {
  ...
  // 有日期的議程自動排進行事曆。
  // 存成 task 而不是 reminder：這些是自動抽出來的，不是使用者明講要提醒的，
  // is_reminded 設 true 讓提醒輪詢跳過，才不會每一筆都推播。
  // 日曆和早安簡報都看得到，所以不會漏掉。
  let agendaText = '';
  const agenda = await extractAgendaItems(content, dateISO);
  if (agenda.length) {
    const batch = randomUUID();
    const rows = agenda.map(a => ({
      raw_text: `[會議議程] ${a.content}`, type: 'task', project: MEETING_PROJECT,
      content: a.content, due_date: a.due_date,
      meta: { agenda_batch: batch },
      is_reminded: true, is_done: false,
    }));
    const { error: agErr } = await supabase.from('notes').insert(rows);
    if (!agErr) {
      // 這些是整天的行程，不需要顯示時間。
      // 不要用 formatTaipeiDate(...).split(' ')：它的日期與時間之間是
      // U+2009 窄空格而不是一般空格，切不開會把「00:00」一起印出來。
      const lines = agenda.map(a => `• ${taipeiKey(a.due_date).slice(5).replace('-', '/')} ${a.content}`);
      agendaText = `\n\n📅 已排進行事曆 ${agenda.length} 筆：\n${lines.join('\n')}`;
      agendaBatch = batch;
    }
  }

  const title = meetingTitle(content);
  const head = title
    ? `✅ 已存「${title}」${fmtOrderDate(dateISO)}`
    : `✅ 會議記錄已存 ${fmtOrderDate(dateISO)}`;
  const text = `${head}\n📝\n${content.trim()}${actionText}${agendaText}`;

  // 自動排進行事曆的東西一定要能一鍵收回，不然抽錯了要一筆一筆刪很痛苦
  return agendaBatch ? buildAgendaUndoReply(text, agendaBatch) : text;
}
...
async function recallMeeting(dateKey) {
  ...
  return formatMeeting(item);
}

function formatMeeting(item) {
  const title = meetingTitle(item.content);
  const head = title
    ? `📝 ${title}　${fmtOrderDate(item.due_date)}`
    : `📝 會議記錄 ${fmtOrderDate(item.due_date)}`;
  return `${head}\n${item.content.trim()}`;
}

async function recallMeetingById(id) {
  const { data } = await supabase.from('notes').select('*')
    .eq('id', id).eq('project', MEETING_PROJECT).maybeSingle();
  if (!data) return '找不到那筆會議記錄，可能已經被刪掉了。';
...
async function listMeetings() {
  ...
  data.slice(0, 12).forEach((n) => {
    const md = taipeiKey(n.due_date).slice(5).replace('-', '/');
    const title = meetingTitle(n.content);
    // 標題比星期幾有用，所以按鈕上省掉星期讓標題有空間
    const label = title ? `${md} ${title}` : `${md} 會議記錄`;
    contents.push({
      type: 'button', style: 'secondary', height: 'sm', margin: 'sm',
      action: {
        type: 'postback',
        label: truncateLabel(label),
        // 用 id 而不是日期：同一天可能有兩場會議（例如上午同工會、晚上小組長月會），
        // 帶日期的話兩顆按鈕會打開同一筆
        data: `act=viewmeeting&id=${n.id}`,
        displayText: `看 ${md} ${title || '會議記錄'}`,
      },
    });
  });
  return {
...
async function dispatch(text) {
  ...
  // 週報存檔：翻前幾週的回顧
  if (/^(?:週報|周報)(?:清單|列表)?$/.test(trimmed)) return listWeeklyReports();
...
async function handlePostback(dataStr) {
  ...
  if (act === 'delagenda') {
    ...
    const { data, error } = await supabase.from('notes').delete()
      .eq('meta->>agenda_batch', batch).select('id');
    if (error) {
      console.error('delagenda error:', error);
      return '取消失敗，稍後再試試。';
    }
    const n = data ? data.length : 0;
    return n ? `🗑 已把 ${n} 筆從行事曆移除。` : '那批項目已經不在了。';
  }
  if (act === 'viewmeeting') {
    // 新的按鈕帶 id；date 是舊按鈕用的，聊天記錄裡的舊訊息還點得到，所以保留
    const mid = p.get('id');
    if (mid) return recallMeetingById(mid);
    const date = p.get('date');
...
// 翻存檔的週報。週報是「那一週當下」的快照，過了就重新生不出來，
// 所以值得存下來，也值得有個地方翻。
async function listWeeklyReports(limit = 5) {
  const { data } = await supabase.from('notes').select('content, created_at')
    .eq('project', REPORT_PROJECT)
    .order('created_at', { ascending: false }).limit(limit);

  if (!data || data.length === 0) {
    return '📊 還沒有存檔的週報。每週五早上會自動產生一份，之後就能在這裡翻。';
  }
  const lines = [`📊 最近 ${data.length} 份週報`, ''];
  data.forEach((r, i) => {
    if (i > 0) lines.push('');
...
async function handleMorning(opts = {}) {
  ...
  let worship = '';
  try {
    const w = await getScheduleByDate(`${mo + 1}/${d}`, { suggestNearest: false });
