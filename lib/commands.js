...
const { addMonthlyReminder, listMonthlyReminders, deleteMonthlyReminder } = require('./monthly');
...
async function dispatch(text) {
  ...
  // Monthly recurring reminders（每月固定提醒）
  if (trimmed.startsWith('刪除月提醒')) return deleteMonthlyReminder(trimmed.slice(5).trim());
  if (trimmed === '月提醒' || trimmed === '每月提醒' || trimmed === '每月提醒清單') {
    return listMonthlyReminders();
  }
  if (trimmed.startsWith('每月')) return addMonthlyReminder(trimmed);
