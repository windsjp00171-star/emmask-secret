...
const HELP_SECTIONS = {
  ...
  worship: [
    ...
    '　（編號歌單「1.歌名」寫法）還會附按鈕選單，點一下看該首完整資料／版本紀錄，沒存過的會提醒你補',
    '',
...
function handleChangelog() {
  return [
    ...
    '・編號歌單（1.歌名）會附歌單按鈕選單：點歌看完整資料/版本紀錄，沒存過的歌會提醒補上',
...
function buildSongMenu(matches, missing) {
  ...
  if (matches.length) {
    ...
    matches.slice(0, 10).forEach((m) => {
      contents.push({
        type: 'button', style: 'secondary', height: 'sm', margin: 'sm',
        action: { type: 'postback', label: `🎼 ${m.title}`, data: `act=songinfo&title=${encodeURIComponent(m.title)}`, displayText: `查 ${m.title} 的資料` },
      });
    });
  }
  if (missing.length) {
    ...
    contents: {
      ...
      body: {
        ...
        contents: [
          ...
          ...history.map((h, i) => ({
            type: 'button', style: 'secondary', height: 'sm', margin: 'sm',
            action: { type: 'postback', label: h.length > 20 ? `${h.slice(0, 20)}…` : h, data: `act=songhist&title=${encodeURIComponent(title)}&idx=${i}`, displayText: `查 ${title} 的舊版本` },
          })),
        ],
      },
    },
...
async function recallOrder(dateKey) {
  ...
  return buildOrderReply(`🎵 敬拜順序 ${fmtOrderDate(item.due_date)}`, item.content);
}
