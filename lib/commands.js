...
async function listThemes() {
  const { data } = await supabase.from('notes').select('*')
    .in('type', ['reminder', 'task']).not('project', 'is', null).eq('is_done', false)
    .order('due_date');
  const themes = {};
  (data || []).forEach(n => {
    if (!n.project || SYSTEM_PROJECTS.includes(n.project)) return;
    if (!themes[n.project]) themes[n.project] = { count: 0, next: n.due_date };
    themes[n.project].count++;
  });
  const names = Object.keys(themes);
  if (names.length === 0) return '目前沒有主題。\n建立：主題 名稱（換行）每行「日期 時間 行程」';
  const lines = ['📋 目前的主題'];
  names.forEach(t => {
    const nx = themes[t].next ? `，下次 ${formatTaipeiDate(themes[t].next)}` : '';
    lines.push(`• ${t}（${themes[t].count} 則${nx}）`);
  });
  lines.push('\n看內容：看主題 名稱');
  return lines.join('\n');
}
