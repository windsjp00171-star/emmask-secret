...
// 抓出「提前N分鐘/N小時/半小時」並從文字中移除，回傳 { lead, text }
...
async function handleSave(text) {
  ...
  return (lead != null ? null : classified.reply) || buildSaveReply(data);
}
...
const HELP_SECTIONS = {
  rec: [
    ...
    '・提前通知：內容後面加「提前N分鐘」／「提前N小時」（不寫預設5分鐘）',
    '　例：提醒 明天下午3點看牙醫 提前1小時',
    '',
    '🔁 重複提醒（響過自動排下次）',
    '・提醒 每天 早上8點 讀經',
    '・提醒 每週三 下午4點 全職同工會',
...
async function handleTheme(rest) {
  ...
  for (const raw of items) {
    ...
    if (!due) { skipped.push(raw); continue; }
    let content = it
