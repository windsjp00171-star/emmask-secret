...
function buildSaveReply(note) {
  ...
  if (note.remind_lead_minutes != null) parts.push(`🔔 提前${leadLabel(note.remind_lead_minutes)}`);
  return parts.join('　');
}
...
const HELP_SECTIONS = {
  ...
  worship: [
    ...
    '・寫順序 [日期] 內容（不填＝下個主日）',
    '・看順序 [日期]（不填＝最近那份）',
    '・順序裡出現的歌名如果詩歌庫有存，會自動帶出調性/BPM/連結',
    '',
    '🎼 詩歌庫（歌曲資料庫，跟日期無關，平常查資料用）',
    '・存歌 歌名 調性 BPM 連結 段落',
    '　同名歌曲會覆蓋更新（後蓋前），調性有變會附上簡易異動紀錄',
    '・找歌 關鍵字 ／ 詩歌庫',
    '',
    '📑 順序範本',
    '・存範本 名稱（換行）流程',
...
async function handleTodos() {
  ...
  const items = (data || []).filter(n => !n.recur);
  ...
  shown.forEach((n, i) => {
    ...
    rows.push({
      ...
      contents: [
        { type: 'text', text: `${when}${n.content}`, size: 'sm', flex: 1, wrap: true,
  ...
  const header = `📋 未完成（${items.length}${items.length > 12 ? '，顯示前 12' : ''}）`;
  return {
    type: 'flex',
    altText: header,
    contents: {
...
// 詩歌庫存的格式：歌名 調性 BPM 連結 段落（空白分隔，段落可含空白）
function parseSongEntry(content) {
  const [main] = content.trim().split(/｜調性紀錄[:：]/); // 忽略已附加的調性紀錄，避免被當成段落內容
  const tokens = main.trim().split(/\s+/);
  return { title: tokens[0] || '', key: tokens[1] || '', bpm: tokens[2] || '', link: tokens[3] || '', section: tokens.slice(4).join(' ') };
}
...
async function matchSongsInOrder(content) {
  ...
  for (const n of data) {
    ...
    if (content.includes(song.title)) { matches.push(song); seen.add(song.title); }
  }
  return matches;
}
...
async function recallOrder(dateKey) {
  ...
  const matches = await matchSongsInOrder(item.content);
  return `🎵 敬拜順序 ${fmtOrderDate(item.due_date)}\n${fmtOrder(item.content)}${fmtSongMatches(matches)}`;
}
...
async function listThemes() {
  ...
  names.forEach(t => {
    ...
    lines.push(`• ${t}（${themes[t].count} 筆${nx}）`);
  });
  lines.push('\n看內容：看主題 名稱');
  return lines.join('\n');
}
...
// ===== 詩歌資料庫 =====
// 同名歌曲：後蓋前（更新既有那筆），並在內容末端附上簡易調性異動履歷
async function addSong(content) {
  ...
  if (prev) {
    ...
    const oldHistoryMatch = prev.content.match(/｜調性紀錄[:：]\s*(.+)$/);
    const historyList = oldHistoryMatch ? oldHistoryMatch[1].split('、') : [];
    if (prevEntry.key && prevEntry.key !== entry.key) {
      const todayLabel = taipeiKey(new Date().toISOString()).slice(5).replace('-', '/');
      historyList.push(`${todayLabel}前:${prevEntry.key}`);
      keyChangeNote = `\n🔁 調性從 ${prevEntry.key} 改成 ${entry.key || '（無）'}`;
    }
    if (historyList.length) historyNote = ` ｜調性紀錄:${historyList.slice(-5).join('、')}`;
  }

  const newContent = `${entry.title} ${entry.key} ${entry.bpm} ${entry.link}${entry.section ? ' ' + entry.section : ''}${historyNote}`.trim();
...
async function dispatch(text) {
  ...
  if (['待辦', '代辦', '未完成', '待辦清單', '未完成清單'].includes(trimmed)) return handleTodos();
...
async function handleMorning() {
  ...
  const { data: overdue } = await supabase
    ...
    .lt('due_date', taipeiToISO(y, mo, d, 0, 0))
    .order('due_date');
  if (overdue && overdue.length) {
    const lines = overdue.slice(0, 10).map(n => `• ${formatTaipeiDate(n.due_date)} ${n.content}`);
    overdueText = `\n\n⚠️ 逾期未完成（${overdue.length}件）\n${lines.join('\n')}`;
  }
