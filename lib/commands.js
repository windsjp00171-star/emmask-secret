...
async function handleNotes() {
  ...
  const data = (raw || []).filter(n => n.project !== ORDER_PROJECT).slice(0, 10);
...
function handleHelp() {
  return [
    ...
    '',
    '🎵 敬拜順序（主領歌單/流程）：',
    '順序 6/29 流程內容 → 記錄當天敬拜順序',
    '看順序 6/29 → 叫出當天（也可：順序 6/29）',
    '看順序 → 叫出最近/即將的那份',
    '',
    '完成 {關鍵字} → 標記完成',
    '刪除 {關鍵字} → 刪除事項',
...
async function handleWorshipOrder(rest) {
  ...
  const { error } = await supabase.from('notes').insert({
    raw_text: rest, type: 'note', project: ORDER_PROJECT, content: songs,
...
async function dispatch(text) {
  ...
  // 敬拜順序 — 明確「看/查」動詞 = 叫出
  const orderView = trimmed.match(/^(?:看|查|叫|顯示)\s*(?:敬拜)?順序\s*([\s\S]*)$/);
  if (orderView) return recallOrder(orderRecallKey(orderView[1].trim()));
  // 「順序」開頭：有內容=寫、只有日期/空白=叫
