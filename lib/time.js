...
// AI 回的 ISO 字串不一定帶時區。像 "2026-09-14T14:30:00" 這種沒有時區的字串，
// new Date() 會當成 UTC 解析，再轉台北時間就整整晚了 8 小時
// （實測 gemini-3.1-flash-lite 就是回沒有時區的格式，海報上的 14:30 變成 22:30）。
// 所以 AI 給的時間一律在這裡補上 +08:00 再寫進資料庫。
function normalizeTaipeiISO(s) {
  if (typeof s !== 'string') return null;
  const v = s.trim();
  if (!v) return null;

  // 已經有時區（Z 或 ±HH:MM / ±HHMM）就原樣保留
