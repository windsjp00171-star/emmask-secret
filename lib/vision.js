    ...
    const resp = await anthropic.messages.create({
      ...
      messages: [{
        ...
        content: [
          ...
          { type: 'text', text: '請擷取圖片中的活動與時間，照規則輸出 JSON。' },
        ],
      }],
    });
    const text = resp.content[0].text.trim();
    const m = text.match(/\{[\s\S]*\}/);
    if (!m) return { found: false, note: '我看了圖片，但讀不太出明確的活動資訊。' };
    return JSON.parse(m[0]);
  } catch (err) {
    console.error('Vision error:', err);
    const msg = String(err.message || err);
    if (/credit balance is too low|billing|quota|insufficient|429/i.test(msg)) {
      return { found: false, note: 'AI 額度不足了（Anthropic 餘額用完）。儲值後就能用圖片辨識；現在可以先用打字記錄。' };
    }
